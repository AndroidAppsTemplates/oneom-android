package com.iam.oneom.env.handling.recycler.layoutmanagers;
 ...
 * <p/>
 * Now it's impossible to run addCart/remove animations with child views which have arbitrary dimensions (height for
...
public class GridLayoutManager extends android.support.v7.widget.GridLayoutManager {
    ...
    private int decorationHeight = 0;
    ...
    public GridLayoutManager(Context context, int spanCount, int decorationHeight) {
        super(context, spanCount);
        this.spanCount = spanCount;
        this.context = context;
    ...
    public GridLayoutManager(RecyclerView view, int spanCount) {
    ...
    public void onMeasure(RecyclerView.Recycler recycler, RecyclerView.State state, int widthSpec, int heightSpec) {
        ...
        final int rowsCount = adapterItemCount % 2 == 1 ? (adapterItemCount + 1) / spanCount : adapterItemCount / spanCount;
        ...
        double pivot = 1 + 1f / adapterItemCount;

        setMeasuredDimension(width, adapterItemCount / 2 == 1 ? (int) (((double) height * pivot) / 2) : height / spanCount);
...
//            OneOm.handleError(Thread.currentThread(), e, "Measure child in GridLayoutManager", "IMPORTANT!", "Handled exception");
        ...
        final int wMargin = p.leftMargin + p.rightMargin;
        final int hMargin = p.topMargin + p.bottomMargin;
        ...
        final int wDecoration = getRightDecorationWidth(child) + getLeftDecorationWidth(child);
        final int hDecoration = getTopDecorationHeight(child) + getBottomDecorationHeight(child);

        final int childWidthSpec = getChildMeasureSpec(widthSize, wPadding + wMargin + wDecoration, p.width, canScrollHorizontally());
        final int childHeightSpec = getChildMeasureSpec(heightSize, hPadding + hMargin + hDecoration, p.height, canScrollVertically());

        child.measure(childWidthSpec, childHeightSpec);

        dimensions[CHILD_WIDTH] = getDecoratedMeasuredWidth(child) + wPadding + wMargin + wDecoration * 2;
        dimensions[CHILD_HEIGHT] = getDecoratedMeasuredHeight(child) + hPadding + hMargin + hDecoration * 2;
