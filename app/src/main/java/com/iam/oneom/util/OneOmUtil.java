...
public class OneOmUtil {
    ...
    public static CharSequence description(Episode episode, String defaultText) {
        if (episode.getDescription() == null || episode.getDescription().size() == 0) {
            return defaultText == null ? "" : Html.fromHtml(defaultText);
        }

        return Html.fromHtml(episode.getDescription().get(0).getBody());
    }
